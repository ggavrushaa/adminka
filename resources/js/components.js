    ...
    var swiperFeature = new Swiper('.content-feature__main', {
        ...
        on: {
            init: function (swiper) {
                document.querySelectorAll('.feature-thumb__items .feature-thumb__item').forEach(item => {
            ...
            slideChangeTransitionStart: function (swiper) {
                ...
                document.querySelector('.feature-thumb__fraction .thumb-number__start').innerHTML = (number < 10) ? `0${number}` : number;
                removeClass(document.querySelectorAll('.feature-thumb__items .feature-thumb__item'), 'active');
                document.querySelector(`.feature-thumb__items .feature-thumb__item[data-index="${number}"]`).classList.add('active');
            },
        },
    });
...
const fixedHeaderTriggerHeight = (homeHeader) ? homeHeader.clientHeight : fixedHeader.clientHeight * 2;
...
function scrollHandler() {
    ...
    } else if (window.scrollY <= fixedHeaderTriggerHeight && fixedHeaderActive()) {
...
function windowOnLoad() {
    ...
    if (tabBlock.length > 0) {
        tabBlock.forEach(tab => {
            ...
            // Navigation
            const btn_navigation = tab.querySelectorAll('.js-btn-prev, .js-btn-next')

            if (btn_navigation.length === 2) {
                btn_navigation.forEach(btn => {
                    btn.addEventListener('click', () => {
                        let next_index = +tab.querySelector('.js-tab-btn.active').dataset.index + 1;
                        if (btn.classList.contains('js-btn-prev')) {
                            next_index = +tab.querySelector('.js-tab-btn.active').dataset.index - 1;
                        }
                        const next_btn = tab.querySelector(`.js-tab-btn[data-index="${next_index}"]`);

                        if (next_btn) next_btn.dispatchEvent(new Event('click'));
                    })
                })
            }
        })
        ...
        function setActiveTab(tab, btn, tabBtns) {
            ...
            tab.style.setProperty('--scrollbar-position', `${btn.offsetLeft}px`);
            ...
            if (btn_pos.right > tab_top_pos.right) {
                scroll = tab_top.scrollLeft + (btn_pos.right - tab_top_pos.right);

                // У випадку якщо таб більший ніж розмір екрану і якщо він після скролу буде виходити за лівий край
                if (btn_pos.width > tab_top_pos.width && btn_pos.left - scroll < tab_top_pos.left) {
                    scroll = scroll - ((btn_pos.left - scroll - tab_top_pos.left) * -1)
                }
                need_scroll = true;
            }
            ...
            if (btn_pos.left < tab_top_pos.left) {
                scroll = tab_top.scrollLeft - (btn_pos.left * -1 + tab_top_pos.left);
                need_scroll = true;
            }
...
if (formTalk) {
    const form = formTalk.querySelector('form'),
        ...
        paginationItemsSeparator = document.querySelectorAll('.form-pagination .pagination-separator');
    ...
    function nextStep() {
        if (currentStepIndex < count - 1 && validateForm(steps[currentStepIndex])) {
            currentStepIndex += 1;
            setStep();
        }
    ...
    btnSubmit.addEventListener('click', () => {
        ...
        if (uploadedFileList.length){
            for (var i = 0; i < uploadedFileList.length; i++) {
                formData.append("hidden-files[]", uploadedFileList[i], uploadedFileList[i].name);
            }
        }
            ...
            if (paginationItems[i]){
                if (i !== currentStepIndex){
                    ...
                    if (paginationItems[i].nextElementSibling){
                        paginationItems[i].nextElementSibling.classList.add('active');
                    }
                }else {
    ...
    // Contact in select
    const radioContactIn = document.querySelectorAll("input[name='contact_in[]']");
    if (radioContactIn.length){
        radioContactIn.forEach(radio => {
            radio.addEventListener('change', () => {
                radio.closest('.radio-input').classList.toggle('active');
                radio.closest('.radio-input').querySelector('input[type="text"]').focus();
    ...
    function handleFiles() {
        ...
        var validation = validationFiles(fileList, ['pdf', 'jpg', 'jpeg', 'png', 'doc', 'docx', 'xls', 'xlsx', 'excel'], 5000000);
    ...
    function handleShowMoreUploader(){
        ...
        if (block){
            const cloneBlock = block.cloneNode(true),
                ...
                index = block.closest('.files-group').querySelectorAll('.form-group').length;
...
if (window.matchMedia('(min-width:1280px)').matches) {
    ...
    sections.forEach((section, index) => {
        ...
        if (section.nextElementSibling){
            margin = section.nextElementSibling.getBoundingClientRect().left - coord.right;
        }
        if (index === 0){
            tee += coord.left * 2;
        }
...
if (window.matchMedia('(min-width:1024px)').matches) {
    ...
    if(caseFixed){
        let j = childElement.getBoundingClientRect().top - targetElement.getBoundingClientRect().top;
            ...
            let tl = gsap.timeline({
                scrollTrigger: {
                    ...
                    onEnter: () => {
                        ...
                        document.querySelector('.case-fixed .case-fraction__number_start').innerHTML = (i < 9) ? `0${i+1}` : i+1;
                    },
                    onLeave: () => {
                    ...
                    onUpdate: function (safe){
                        ...
                        caseFixed.style.top = `${t.getBoundingClientRect().top - caseFixed.clientHeight}px`;
    ...
    if (!supportsTouch){
        reviewScrollTrigger = gsap.to('.review-card', {
            y: function (i, target, parent){
                let g = target.closest('.tab-bottom__item').querySelector('.col').getBoundingClientRect().height;
            ...
            scrollTrigger: {
                ...
                scrub: 1,
                    ...
                    let trigger = i.trigger.querySelector('.tab-bottom__item.active .col');
        ...
        let tabReviewBtns = document.querySelectorAll('.section-reviews .js-tab-btn');
...
if (document.querySelector('.section-reviews')){
    let ggg = document.querySelector('.section-reviews').getBoundingClientRect().top;
}
...
function updateScrollTriggerReview(btn){
    ...
    let activeTabItem = document.querySelector(`.section-reviews ._js-tab-${btn.dataset.index}`);
    ...
    reviewScrollTrigger = gsap.to('.tab-bottom__item.active .review-card', {
        ...
        scrollTrigger: {
            ...
            scrub: 1,
                ...
                console.log(i.trigger.querySelector('.tab-bottom__item.active .col'))
...
if (solution_swiper_blocks.length > 0){
    solution_swiper_blocks.forEach(block => {
        if (block.querySelector('.solution-item__swiper') && block.querySelector('.solution-item__swiper-thumbs')){
            const thumbSlides = block.querySelectorAll('.solution-item__swiper-thumbs .swiper-slide');
            ...
            let mainSwiper = new Swiper(block.querySelector('.solution-item__swiper'), {
                ...
                on: {
                    slideChange: function (swiper) {
                        ...
                        thumbSlides[swiper.activeIndex].classList.add('active');
                    },
                },
            })
...
// Contact page, form
// select direction
let select_contact_direction__node = document.querySelector('#direction-contacts');
